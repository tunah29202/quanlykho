<template>
    <div class="vc-page page-package">
        <vc-card>
            <h1 class="pb-4" style="border-bottom: 1px solid #cdcdcd">
                {{ tl("Common", "manage_text", [tl("package", "package_text")]) }}
            </h1>
            <vc-row :gutter="12" class="mt-4">
                <vc-col :span="8">
                    <el-input v-model="search" :placeholder="tl('Common', 'input_search_holder', [tl('package', 'package_text')])" :prefix-icon="Search" @keyup.enter="onSearch" />
                </vc-col>
                <vc-col :span="8">
...
<script setup lang="ts">
    ...
    import { colConfig, tableConfig } from '@/commons/config/app/package.config'
    import { usePackageStore } from '@app/stores/app/package.store'
    ...
    const onAddNew = () => {
        ...
        detailRef.value.open(tl("Common", "title_modal_add", [tl("package", "package_text")]), null, async (res: any) => {
    ...
    const onEdit = (item: any) => {
        ...
        detailRef.value.open(tl("Common", "title_modal_edit", [tl("package", "package_text")]), item.id, async (res: any) => {
            if (res) await onSearch()
        })
    };
    ...
    const onView = (item: any) => {
        ...
        detailRef.value.open(tl("Common", "title_modal_detail", [tl("package", "package_text")]), item.id)
    };

    const onDeleteItem = (item: any) => {
        confirmDialog.value.confirm(
            tl("Common", "title_modal_delete"),
            tl("Common", "comfirm_delete", [tl("package", "package_text")]),
            async (res: any) => {
                if (res) {
