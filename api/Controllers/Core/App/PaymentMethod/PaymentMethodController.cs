...
namespace Controllers.Core
{
    ...
    {
        private IPaymentMethodServices paymentMethodServices { get; set; }
        private readonly ILocalizeServices ls;
        public PaymentMethodController(IPaymentMethodServices _paymentMethodServices, ILocalizeServices _ls) : base()
        {
            paymentMethodServices = _paymentMethodServices;
            ls = _ls;
        }
        ...
        {
            var data = await paymentMethodServices.GetAll(request);
            return Ok(data);
        }
        ...
        {
            var data = await paymentMethodServices.GetById(id);
            if (data != null)
            {
        ...
        {
            int count = await paymentMethodServices.Create(request);
            if (count >= 1)
            {
        ...
        [HttpPut]
        [Route("update/{id}")]
        public async Task<IActionResult> Update(Guid id, [FromBody] PaymentMethodRequest request)
        {
            int count = await paymentMethodServices.Update(id, request);
            if (count >= 1)
            {
        ...
        {
            int count = await paymentMethodServices.Delete(id);
            if (count >= 1)
            {
